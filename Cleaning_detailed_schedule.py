def clean_detailed_schedule(raw_path,tbl_schedule_path,out_path):
    #cleans tbl_schedule_01.csv, the detailed schedule file.
    #raw_path: path to raw data
    #tbl_schedule_path: path to tbl_schedule_01.csv --for some reason this raw file was in a different folder
    #outpath: path to files we created 
    ...
    sched_tbl = pd.read_csv(tbl_schedule_path + '/tbl_schedule_01.csv', header=None, names=cols, encoding='ISO-8859-1', low_memory=False, dtype={'adj_date':'str', 'adj_time_start':'str', 'adj_time_stop':'str'})
    ...
    # ====== LANGUAGE OF HEARING =======
    lang_map = pd.read_excel(out_path+'/tbl_language.xlsx')
    ...
    schedtype_map = pd.read_csv(raw_path + '/tbllookupSchedule_Type.csv', header=None)
    ...
    # MERGING notice_code  ONTO sched_tbl
    notice_map = pd.read_csv(raw_path + '/eoir/tblLookupNOTICE.csv', header=None)
    ...
    #change "0" values to not a time (NaT)
    ...
    adjrsn_map = pd.read_excel(raw_path+'/dbo_tblAdjournmentcodes.xlsx')
    ...
    # ====== OUTPUT CLEANED FILE ======
    sched_tbl.to_csv(out_path + '/detailed_schedule_final.csv', index=False)
