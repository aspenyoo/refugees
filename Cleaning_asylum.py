def clean_master_app(inpath,outpath,flag_full):
    # #### This script cleans and merges relevant variables across datasets
    # #### inpath: folder where raw data are
    # #### outpath: folder to put cleaned data
    ...
    app = pd.read_csv(inpath + '/court_appln.csv', low_memory=False)
    app.to_csv(outpath + '/blatest.csv',index=False)
    ...
    # load in data
    master = pd.read_csv(inpath + '/master.csv', low_memory=False)
    ...
    #attorney_flag has values 1 (present) or nan (not present). replace nan attorney flags with 0.
    ...
    hearingloc_map = pd.read_csv(inpath + '/tblLookupHloc.csv', header=None)
    ...
    basecity_map = pd.read_csv(inpath + '/tblLookupBaseCity.csv', header=None)
    ...
    nat_lut =  pd.read_csv(inpath+ '/tblLookupNationality.csv',header=None) # load lookup table
    ...
    # ====== SAVE DATA ======
    merged_case.to_csv(outpath + '/merged_master_app_' + filetag + '_final.csv',index=False)
