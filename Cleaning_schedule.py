def clean_schedule(raw_path,out_path,flag_early,flag_full):
    #raw_path: path to raw data
    #out_path: path to files we created 
    #flag_early: cleans for early predictability model. if 0, cleans for late predictability model.
    ...
    # ====== master_app ======
    master_app = pd.read_csv(out_path + '/merged_master_app_' + tag_asyl + '.csv')

    # ====== MERGE schedule.csv AND tbllookupSchedule_Type.csv ======
    # load schedule.csv
    sched = pd.read_csv(raw_path + '/schedule.csv', low_memory=False)
